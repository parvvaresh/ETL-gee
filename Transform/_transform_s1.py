...
def transform_sentinel1(sentinel1):

  vvIw = sentinel1.filter(ee.Filter.listContains('transmitterReceiverPolarisation', 'VV')).select(['VV']) \
  ...
  vhIw = sentinel1.filter(ee.Filter.listContains('transmitterReceiverPolarisation', 'VH')).select(['VH']) \
            .filter(ee.Filter.eq('instrumentMode', 'IW'))
